...
--            dateadd(hour, -7, '2019-09-01 06:15:00') as end_,
  ...
  join _ ON start_ <= StartAt AND StartAt < end_
    ...
    from (SELECT * FROM ActivityLog
          ...
          UNION ALL SELECT 7118 as MappedTypeID, WorkerID, timing, time_at FROM ActivityLog where MappedTypeID in (110, 7, 10, 5, 9)
         ) __,_
), timing AS (
    select
...
WHERE PEGroup.PEGroupID in (4)  --lnd: 2 HN & 4 
    AND ActivityType.MappedTypeID = 7118
