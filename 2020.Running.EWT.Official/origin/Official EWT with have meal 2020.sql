...
with _ as (
    select dateadd(hour, -7, cast(cast(cast(dateadd(day, 25, date_trunc('month',
              current_date-150/*SUA NGAY THANG TAI DAY*/)) as date) AS varchar) + ' 06:15:00' as datetime)) as time_
), filter_worker AS (
    SELECT DISTINCT
...
--     WHERE ClientTimeReportAt >= '2019-07-16 23:15:00' AND ClientTimeReportAt < '2019-07-17 23:15:00'
    join _ ON ClientTimeReportAt >= dateadd(month, -1, _.time_)
...
--            where RequestDatetimeUtc >= '2019-07-16 23:15:00' and RequestDatetimeUtc < '2019-07-17 23:15:00'
           join _ ON RequestDatetimeUtc >= dateadd(month, -1, _.time_)
  ...
  StartAt as time_at
  from WorkerMeals
 join _ ON StartAt >= dateadd(month, -1, _.time_)
...
--           ShiftStartDateLocalTime < dateadd(hour, +7, '2019-07-17 23:15:00')
    join _ on ShiftStartDateLocalTime >= dateadd(month, -1, dateadd(hour, +7, _.time_))
