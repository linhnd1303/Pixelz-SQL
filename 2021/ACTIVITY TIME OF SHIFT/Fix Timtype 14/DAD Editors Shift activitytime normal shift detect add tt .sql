...
** 3    2021-03-19  Vu          add timetype 14  
*******************************************************************************************/
with _ as (
    select
...
--            dateadd(hour, -7, '2019-09-01 06:15:00') as end_,
    ...
    from (select dateadd(hour, -7, dateadd(minute, mins_, date_trunc('month',
    ...
    group by ProductionWorkers.PlanningGroupID
      ...
      UNION ALL SELECT 10   , '1__PostProcessingTime_full'
      UNION ALL SELECT 14   , '1__PostProcessingTime_semi'
      ...
      UNION ALL SELECT 1111 , 'ProductionTime'
  	  UNION ALL SELECT 2222 , 'Non_ProductionTime'
), ActivityLog AS (
    SELECT
      CASE WHEN (isnull(ImageID, 0) > 0 AND (EditorClientLoginTypeID = 0 OR ActivityTimeTypeID = 0)) THEN 110 ELSE
      CASE WHEN ActivityTimeTypeID in (7, 1, 9, 10,14, 6, 5, 2, 3 ,4, -1, 8) THEN ActivityTimeTypeID ELSE
  ...
  join _ ON start_ <= StartAt AND StartAt < end_
    ...
    from (SELECT * FROM ActivityLog
          ...
          UNION ALL SELECT 7117 as MappedTypeID, WorkerID, timing, time_at FROM ActivityLog where MappedTypeID in (110, 10,14, 5)
          UNION ALL SELECT 1111 as MappedTypeID, WorkerID, timing, time_at FROM ActivityLog where MappedTypeID in (1, 411, 1312, 514, 310, 1001, 1091, 1092)
          UNION ALL SELECT 2222 as MappedTypeID, WorkerID, timing, time_at FROM ActivityLog where MappedTypeID in (110, 7, 10 , 14, 6, 5, 9)
...
WHERE filter_worker.PEGroupID in (4)
      and ActivityType.MappedTypeID in (1111,2222)
