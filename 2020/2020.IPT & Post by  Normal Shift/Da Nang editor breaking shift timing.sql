...
--            dateadd(hour, -7, '2019-09-01 06:15:00') as end_,
    ...
    SELECT
      ...
      CASE WHEN (UPPER(ApplicationName) LIKE '%PHOTO EDITOR CLIENT%'
        ...
        OR lower(ApplicationName) like '%smooth_path%') THEN 1001 ELSE --'WorkingTool'
      CASE WHEN (UPPER(ApplicationName) LIKE '%SKYPE%') THEN 514 ELSE --'Skype'
      CASE WHEN (UPPER(ApplicationName) LIKE 'IDLE%') THEN 1013 ELSE --'IDLE'
        CASE WHEN (UPPER(ApplicationName) LIKE '%EXPLORER%') THEN 310 ELSE --'EXPLORER'
      CASE WHEN (UPPER(ApplicationName) LIKE 'CHROME%' OR
                  UPPER(ApplicationName) LIKE 'GOOGLE CHROME%' OR
          ...
          ApplicationName LIKE 'Opera Internet Browser' OR
                  UPPER(ApplicationName) LIKE 'BROWSER%') THEN 1312 ELSE 411 --1312 'BROWSER' 411 others
      END END END END END END END AS MappedTypeID,
      WorkerID,
...
WHERE filter_worker.PEGroupID in (4) and ActivityType.MappedTypeID in (1, -1, 1013)
